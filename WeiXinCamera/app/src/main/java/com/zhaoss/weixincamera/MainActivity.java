...
public class MainActivity extends Activity implements View.OnClickListener, SurfaceHolder.Callback {
    ...
    /** mediaRecorder是否已经初始化完毕 */
    private boolean isMediaRecorderInitOk;
    ...
    private void initMediaRecorder() {
        ...
        mediaRecorder.setOutputFile(createCameraFile().getPath());
        mediaRecorder.setPreviewDisplay(holder.getSurface());
        try {
            mediaRecorder.prepare();
            isMediaRecorderInitOk = true;
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
